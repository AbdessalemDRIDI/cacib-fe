...
/**
 * MessageMessageSearchBaseService
 *
 * This service extends the ScreenService and provides functionality
 * for managing data related to the 'Message-MessageSearch' screen.
 */
export class MessageMessageSearchBaseService extends ScreenService {
  data$ = new BehaviorSubject(undefined);
  pageSize = signal(10);

  constructor() {
    super();
    this.screenId = 'Message-MessageSearch';
  }
  ...
  count(payload: Payload) {
    ...
    return this.httpClient.get<any>(`${this.BASE_PATH}/messages/count`, { params }).pipe(
      tap((result) => {
  ...
  search(payload: Payload) {
    ...
    return this.httpClient.get<any>(`${this.BASE_PATH}/messages/search`, { params }).pipe(
      tap((data) => {
        this.loading.set(false);
  ...
  executeDelete(context): Observable<any> {
    const params = {};
    return this.httpClient.delete<any>(`${this.BASE_PATH}/messages/{code}`, { params });
  }
  /**
  ...
  export(payload) {
    ...
    return of(payload).pipe(
      ...
      tap(([result, payload]) => {
        ...
        const fileName = 'Message' + fileExtension;
  ...
  executeExport(context): Observable<any> {
    ...
    const currentKeys = `id,messageContent,messageType,dateReceived,status`;
