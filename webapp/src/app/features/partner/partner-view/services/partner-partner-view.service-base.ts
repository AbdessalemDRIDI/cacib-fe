...
/**
 * PartnerPartnerViewBaseService
 *
 * This service extends the ScreenService and provides functionality
 * for managing data related to the 'Partner-PartnerView' screen.
 */
export class PartnerPartnerViewBaseService extends ScreenService {
  constructor() {
    super();
    this.screenId = 'Partner-PartnerView';
  }
  ...
  initValue(payload: Payload): Observable<any> {
    ...
    if (payload.parentId && (payload.roleName || payload.code !== undefined)) {
      ...
      return this.httpClient.get<any>(`${this.BASE_PATH}/partners/${code}`, { params }).pipe(
        tap((result) => {
